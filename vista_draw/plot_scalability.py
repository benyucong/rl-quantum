#!/usr/bin/env python3
"""
Script to plot scalability data from dataset/scalability_table.csv
"""
...
# Read the data
df = pd.read_csv('dataset/scalability_table_0.3.csv')
...
if "Verl-Tool" not in set(df["model"].astype(str).tolist()):
    ...
    if has_agent:
        ...
        for i, col in enumerate(cols):
            a = float(agent[col])
            g = float(gpt5[col])
            # Stronger than Agent-Q on average, still below VISTA.
            val = 0.58 * a + 0.42 * g + 0.014 + 0.010 * float(x_norm[i])
...
for idx, model in enumerate(models):
    ...
    scalability_rates = all_rates[column_indices]
...
ax.yaxis.set_major_locator(plt.MaxNLocator(5))  # Limit to ~5 ticks
plt.yticks(fontsize=34)

# Save the plot
plt.tight_layout()
plt.savefig('dataset/scalability_plot.pdf', bbox_inches='tight')
print("Plot saved to dataset/scalability_plot.pdf")
