set -x
dataset_name=rl-qasm  # or qasm-mini 
train_data=$(pwd)/data/rl-qasm/graph-data-quantum-rl-linus/train.parquet
val_data=$(pwd)/data/rl-qasm/graph-data-quantum-rl-linus/test.parquet
...
use_fused_kernels=True # use fused kernels for faster training
fsdp_size=-1
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "quantum_cpu" --workers_per_tool 8 &
server_pid=$!
