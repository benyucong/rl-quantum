...
\midrule
End-to-end quality & Table 1 reports Vista Pass@1 SCR 99.31\%, SREV 22.41\%, RE 11.61, HQCR 17.24\%; Pass@10 SCR 100\%, SREV 33.10\%, RE 8.48, HQCR 27.24\%. Recompute \codepath{summary_stats_*.json} from raw generation JSONs with \codepath{evaluate_samples.py}. \\
\midrule
Reward ablation & Table 2 reports full Vista is best overall and removing RE causes the largest drop. Evaluate the full and ablation raw outputs with the same evaluator. \\
\midrule
...
\mypar{Figure mapping}
The exact Fig.-to-artifact correspondence is documented in \codepath{artifact/tables/figure_artifact_map.csv} and \codepath{artifact/README.md}. In short, Figs. 2--6 map to the per-primitive, robustness, scalability, and training-dynamics plot tasks; Figs. 7--9 map to verifier-efficiency, training-log, and stage-cost tasks; and Figs. 10--14 map to archived hardware/QPU postprocessing tasks. Figs. 1 and 15 are design/provenance figures and are supported by code and dataset inspection rather than a model-result plot.

\mypar{Metric recomputation}
To recompute Table 1/Table 2 metrics from raw generated outputs, run:
\begin{verbatim}
