OPENQASM 3.0;
include "stdgates.inc";
bit[11] c;
qubit[11] q;

// Layer 1: Initialization and entanglement
h q[0];
h q[1];
h q[2];
h q[3];
h q[4];
h q[5];
cx q[0], q[5]; // Capacity from source 0 to sink 5
cx q[2], q[3]; // Edge from 2 to 3
cx q[3], q[1]; // Edge from 3 to 1
cx q[3], q[2]; // Edge from 3 back to 2
cx q[3], q[5]; // Edge from 3 to 5
cx q[4], q[2]; // Edge from 4 to 2

// Layer 2: Apply rotation gates to represent edge capacities
rz(0.1273) q[0];
rz(0.2581) q[1];
rz(0.3341) q[2];
rz(0.1943) q[3];
rz(0.0732) q[4];
rz(0.4590) q[5];

h q[0];
h q[1];
h q[2];
h q[3];
h q[4];
h q[5];

// Layer 3: Measurement preparation
cx q[0], q[5]; // Final decision gates
cx q[2], q[3];
cx q[3], q[1];
cx q[3], q[2];
cx q[3], q[5];
cx q[4], q[2];

c[0] = measure q[0];
c[1] = measure q[1];
c[2] = measure q[2];
c[3] = measure q[3];
c[4] = measure q[4];
c[5] = measure q[5];
c[6] = measure q[6];
c[7] = measure q[7];
c[8] = measure q[8];
c[9] = measure q[9];
c[10] = measure q[10];