OPENQASM 3.0;
include "stdgates.inc";
bit[8] c;
qubit[8] q;

// Layer 1: Initialize qubits for the source node (0)
h q[0];
h q[1];
h q[2];
h q[3];

// Layer 2: Apply controlled operations to model the edges
cx q[0], q[1];  // Edge from 0 to 1
cx q[0], q[2];  // Edge from 0 to 2
cx q[0], q[5];  // Edge from 0 to 5
cx q[0], q[6];  // Edge from 0 to 6
cx q[1], q[2];  // Edge from 1 to 2
cx q[1], q[5];  // Edge from 1 to 5
cx q[5], q[3];  // Edge from 5 to 3
cx q[5], q[4];  // Edge from 5 to 4
cx q[6], q[3];  // Edge from 6 to 3
cx q[4], q[7];  // Edge from 4 to 7

// Layer 3: Finalize the circuit with Hadamard gates for measurement basis change
h q[0];
h q[1];
h q[2];
h q[3];

// Measure the qubits
c[0] = measure q[0];
c[1] = measure q[1];
c[2] = measure q[2];
c[3] = measure q[3];
c[4] = measure q[4];
c[5] = measure q[5];
c[6] = measure q[6];
c[7] = measure q[7];