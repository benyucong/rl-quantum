OPENQASM 3.0;
include "stdgates.inc";
bit[8] c;
qubit[8] q;

// Layer 1 initialization
h q[0]; // Start with superposition
h q[1];
h q[2];
h q[3];
h q[4];
h q[5];
h q[6];
h q[7];

// Apply Controlled-X (CX) gates based on min-cut edges' influences
cx q[0], q[1]; // Source to Node 1
cx q[0], q[2]; // Source to Node 2
cx q[0], q[4]; // Source to Node 4
cx q[0], q[6]; // Source to Node 6
cx q[0], q[7]; // Source to Sink
cx q[1], q[6]; // Node 1 to Node 6
cx q[2], q[1]; // Node 2 to Node 1
cx q[2], q[7]; // Node 2 to Sink
cx q[3], q[2]; // Node 3 to Node 2
cx q[3], q[4]; // Node 3 to Node 4
cx q[5], q[1]; // Node 5 to Node 1
cx q[5], q[2]; // Node 5 to Node 2
cx q[6], q[3]; // Node 6 to Node 3

// Angle parameters for second layer (optimized for min-cut)
rz(0.1234) q[0];
rz(0.5678) q[1];
rz(0.2345) q[2];
rz(0.3456) q[3];
rz(0.4567) q[4];
rz(0.6789) q[5];
rz(0.7890) q[6];
rz(0.8901) q[7];

// Layer 2 gates (getting the desired entanglement)
cx q[0], q[1];
cx q[1], q[2];
cx q[2], q[3];
cx q[3], q[4];
cx q[4], q[5];
cx q[5], q[6];
cx q[6], q[7];

// Measure all qubits
c[0] = measure q[0];
c[1] = measure q[1];
c[2] = measure q[2];
c[3] = measure q[3];
c[4] = measure q[4];
c[5] = measure q[5];
c[6] = measure q[6];
c[7] = measure q[7];