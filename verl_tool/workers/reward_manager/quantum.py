...
from .reward_score.quantum_score import syntax_reward, parse_reward
...
class QuantumRewardManager:
    ...
    def __init__(self, tokenizer, num_examine, compute_score=None, reward_fn_key='data_source') -> None:
        ...
        self.response_validity = syntax_reward
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        if not hasattr(self, 'record_dir'):
            if hasattr(self, 'run_id'):
                ...
                self.record_dir = Path(__file__).parent.parent.parent.parent / "verl_step_records" / f"quantum-{time.strftime('%Y-%m-%d-%H-%M-%S')}"
                self.record_dir.mkdir(parents=True, exist_ok=True)
        ...
        for i in range(len(data)):
            ...
            score['validity'] = self.response_validity(response_str)
            score['score'] = qasm_score
            ...
            if isinstance(score, dict):
                ...
                if self.num_examine == 1:
                    reward = score["validity"] # for validation
            else:
                if self.num_examine == 1:
        ...
        if save_record:
            ...
            if self.num_examine == 1:
                temp_file = self.record_dir / f"quantum-step-val-{self.step}.json"
            else:
                temp_file = self.record_dir / f"quantum-step-{self.step}.json"
            self.step += 1
