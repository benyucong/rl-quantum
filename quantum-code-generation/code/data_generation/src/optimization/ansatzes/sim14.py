    ...
    def get_circuit(self):
        ...
        def circuit(single_qubit_params, two_qubit_params):
            for d in range(self.n_layers):
                ...
                qml.CRX(two_qubit_params[d][0][0], wires=[self.num_qubits - 1, 0])
                
                for i in reversed(range(self.num_qubits - 1)):
                    qml.CRX(two_qubit_params[d][i + 1][0], wires=[i, i + 1])
                
                for i in range(self.num_qubits):
                    qml.RY(single_qubit_params[d][i][1], wires=i)
                
                qml.CRX(two_qubit_params[d][0][1], wires=[self.num_qubits - 1, self.num_qubits - 2])
                qml.CRX(two_qubit_params[d][1][1], wires=[0, self.num_qubits - 1])
                
                for i in range(self.num_qubits - 2):
                    qml.CRX(two_qubit_params[d][i + 2][1], wires=[i, i + 1])
